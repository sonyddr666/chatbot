...
- O endpoint legado `/api/v1/upload` continua sendo o caminho imediato para anexos do chat.
...
```powershell
python -m unittest tests.test_userspace tests.test_workspace_service tests.test_workspace_routes tests.test_ingestion tests.test_rag_isolation tests.test_skill_runtime tests.test_auth_required tests.test_frontend_documents_panel
```

Resultado consolidado: `47` testes passaram em `4.371s`.
